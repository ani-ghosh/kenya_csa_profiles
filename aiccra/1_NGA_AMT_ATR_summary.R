...
makeCountyPlot <- function(i, adm, ff){
  ...
  # past trends
  ht <- read.csv("aiccra/Temperature-Air-2m-Mean.csv", encoding = "UTF-8", stringsAsFactors = FALSE)
  hp <- read.csv("aiccra/precipitation_flux.csv", encoding = "UTF-8", stringsAsFactors = FALSE)
  ...
  # summarize data
  d2$era <- factor(d2$era, levels = c("historical","2021-2040", "2041-2060"))
  ...
  plot2 <- ggplot(d2, aes(x = era, y=AMT, fill=era)) +
    ...
    scale_fill_manual(values = c("#fecc5c", "#fd8d3c", "#e31a1c"), 
                      ...
                      breaks = c("historical","2021-2040", "2041-2060")) +
  ...
  plot4 <- ggplot(d2, aes(x=era, y=ATR, fill=era)) +
    ...
    scale_fill_manual(values = c("#a6bddb", "#74a9cf", "#3690c0"), 
                      aesthetics = c("color", "fill"),
                      breaks = c("historical","2021-2040", "2041-2060")) +
    theme(text = element_text(size = 16),
          axis.title.x = element_blank(),
...
# input
adm <- data.frame(iso3 = "NGA", region = c("Adamawa","Borno","Yobe"))

cdir <- "C:/Users/anibi/Documents/ciat/cmip6"

ff <- list.files(cdir, glob2rx(paste0("NGA*bioc*ssp585*.tif")), full.names = TRUE)
gcm <- c("ACCESS-CM2", "INM-CM5-0", "HadGEM3-GC31-LL", "IPSL-CM6A-LR", "CNRM-CM6-1-HR")
gcms <- paste(gcm, collapse = "|")
ff <- grep(gcms, ff, value = TRUE)
