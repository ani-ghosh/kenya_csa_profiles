...
makePlotSingle <- function(f, iso, vs, pdir){
  ...
  if(grepl("temp_only", f)){mdir <- file.path(mdir, "temp_only")}
  dir.create(mdir, F, T)
  mfile <- file.path(mdir, basename(f))
  ...
  if(!file.exists(mfile)){
    ...
    rr <- terra::disaggregate(rs, fact = 5)
    v00 <- terra::vect(v0)
    rr <- terra::mask(rr, v00, filename = mfile)
  }  
  rr <- stack(mfile)
  
  # mask by irrigation areas for temp_only layers  
  if(grepl("temp_only", f)){
    irr <- file.path("suitability/maps", iso, "irrigation_extent.tif")
    gr <- stack(irr)
    gr <- disaggregate(gr, fact = 5)
    gr <- crop(gr, rr)
    rr <- crop(rr, gr)
    ok <- try(compareRaster(gr,rr), silent = TRUE)
    if(inherits(ok, "try-error")){
      gr <- resample(gr, rr, method='bilinear')
    }
    rr <- mask(rr, gr, maskvalue = 0)
    rr[is.na(rr[])] <- 0
    rr <- mask(rr, v0)
  }
  ...
  if(grepl("temp_only", f)){onames <- paste0(onames, "_", "temp_only")}
  
  cname2 <- file.path(pdir, paste0("suitability_group_maps_", onames, ".png"))
  ...
  cname3 <- file.path(pdir, paste0("suitability_change_analysis_", gsub(".tif",".png",basename(f))))
  ...
  if (nrow(vs)>1){
    ...
    png(cname3, width = 10, height = 8, units = "in", res = 300)
    grid.newpage()
...
makePlots <- function(f, iso, vs, outdir){
  ...
  # data/outputfolder
  fdir <- file.path(outdir, "maps", iso)
  # create a png folder
  pdir <- file.path(fdir, "png")
  dir.create(pdir, showWarnings = FALSE, recursive = TRUE) 
  
  # makePlotSingle(f, iso, vs, pdir)  
  tryCatch(makePlotSingle(f, iso, vs, pdir), error=function(e) NULL)  
  # LZ boundary
  # testPlotFun <- function(f,iso,vs,pdir) {
  #  return(tryCatch(makePlotSingle(f, iso, vs, pdir), error=function(e) NULL))
  #}
  ...
  # for (f in ff){
  #  cat("processing ", f, "\n")
  #  lapply(f, testPlotFun, iso, vs , pdir)
  #}
}
...
v <- shapefile("suitability/all_zones_countries.shp")
outdir <- "suitability"
...
for (iso in isol){
  ...
  ff <- grep(iso, fff, value = TRUE)  
  
  lzname <- file.path("suitability/maps", iso, "lz_boundaries_dissolved.shp")
  dir.create(dirname(lzname), FALSE, TRUE)
  
  if(!file.exists(lzname)){
    vs <- v[v$iso3 == iso,]
    # fix boundary
    vs <- rgeos::gBuffer(vs, byid = T, width = 0.1)  
    vs <- aggregate(vs, "zone")
    vs$pnames <- substr(vs$zone, 1, 15) 
    
    rc <- "+proj=longlat +datum=WGS84 +no_defs"
    if(!compareCRS(vs, rc)){
  ...
  if(!file.exists(irr)){
    ...
    v0 <- getData("GADM", country = iso, level = 0, path = "suitability/maps") 
    gr <- crop(gmia, extent(v0))
    ref <- getData('worldclim', var="tmin", res="2.5", path = "/cluster01/workspace/CCI/Data/CMIP5")
    ref <- crop(ref, extent(v0))
    gr <- disaggregate(gr, 2)
    gr <- crop(gr, ref)
    extent(gr) <- extent(ref)
    gr <- mask(gr, v0, filename = irr)
  }      
  # makePlots(ff, iso, vs, outdir)
